...
module.exports = {
    ...
    async getLastWithDiff(req, res) {
        let currentReport = await DailyReport.findOne({
            limit: 1,
            order: [['reportDate', 'DESC']]
        });

        let previousReport = await DailyReport.findOne({
            limit: 1,
            offset: 1,
            order: [['reportDate', 'DESC']]
        });

        let dailyReportWithDiff = {
            id: currentReport.id,
    ...
    getPDf(req, res) {
        let data = fs.readFileSync('./pdf/' + req.params.filename);
