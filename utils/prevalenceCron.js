...
module.exports = {
    ...
    async prevalenceComputeV2() {
         var url = 'https://services7.arcgis.com/Z6qiqUaS6ImjYL5S/arcgis/rest/services/78_DS_MSAS_mise_%C3%A0_jour/FeatureServer/0/query?f=json&where=1%3D1&returnGeometry=false&spatialRel=esriSpatialRelIntersects&outFields=*&orderByFields=Cas%20desc&outSR=102100&resultOffset=0&resultRecordCount=78&resultType=standard&cacheHint=true';
        https.get(url, (res) => {
            let body = "";
            ...
            res.on("end", async () => {
                try {
                    ...
                    for (var district of features) {
                        prevdata = {
                            numberOfConfirmedCases: district.attributes.Cas,
                            numberOfSupectedCases: 0,
                            numberOfContactsCases: 0,
                            numberOfRecoveredCases: 0, 
                            idZone: '',
    ...
    prevalenceCron() {
        var job = new CronJob('* */300 * * * *', async function () {
            ...
            await module.exports.prevalenceComputeV2();
